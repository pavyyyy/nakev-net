...
function setFiles(files) {
  ...
  els.previewBody.innerHTML = selectedFiles.map((file) => (
    `<tr><td>${escapeHtml(file.name)}</td><td>${escapeHtml(groupFromName(file.name))}</td><td>Ready</td></tr>`
  )).join("");
...
async function generate() {
  ...
  try {
    ...
    for (const file of selectedFiles) {
      const result = await convertAccountingFile(file, { convertDocumentNumber: true, datesAsText: true });
      const item = {
        group: groupFromName(file.name),
    ...
    const outputs = processed.map((item) => ({
      fileName: item.fileName,
      bytes: writeWorkbook(item.workbook),
      rowCount: item.rowCount,
      group: item.group,
    }));

    for (const [left, right] of PAIRS) {
      const pairItems = [byGroup.get(left), byGroup.get(right)].filter(Boolean);
      if (!pairItems.length) continue;
      const pairWorkbook = XLSX.utils.book_new();
      pairItems.forEach((item) => {
        const sheet = XLSX.utils.aoa_to_sheet(item.rows);
        sheet["!cols"] = [{ wch: 24 }, { wch: 20 }, { wch: 12 }, { wch: 10 }, { wch: 14 }, { wch: 14 }];
        XLSX.utils.book_append_sheet(pairWorkbook, sheet, item.group);
      });
      outputs.push({
        fileName: `${left}_${right}.xlsx`,
        bytes: writeWorkbook(pairWorkbook),
        rowCount: pairItems.reduce((sum, item) => sum + item.rowCount, 0),
        group: `${left}_${right}`,
      });
    }
    ...
    if (outputs.length === 1) {
      ...
      downloadBlob(blob, `zaplashtania-exceli-${timestampForName()}.zip`);
    }
...
function renderResults(outputs) {
  els.previewBody.innerHTML = outputs.map((item) => (
    `<tr><td>${escapeHtml(item.fileName)}</td><td>${escapeHtml(item.group)}</td><td>${item.rowCount}</td></tr>`
  )).join("");
}
...
async function convertAccountingFile(file, options) {
  ...
  for (let r = 0; r <= range.e.r; r += 1) {
    ...
    if (documentNumber != null && String(documentNumber).slice(0, 3) === group) firm = documentNumber;
